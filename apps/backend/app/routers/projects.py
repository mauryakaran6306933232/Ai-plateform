...
def get_chroma_client():
    """Get ChromaDB client using same strategy as MemoryService"""
...
async def delete_project(project_id: int, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        logger.warning(f"Failed to delete ChromaDB collection for project {project_id}: {e}")
    ...
    return {
        ...
        "message": "Project and vector memory deleted successfully",
    }
...
async def github_clone(data: dict, db: AsyncSession = Depends(get_db)):
    ...
    project = Project(
        name=repo_name, status="analyzing",
        language="auto-detect", repo_url=repo_url,
    )
    db.add(project)
    await db.commit()
    await db.refresh(project)
...
async def create_pull_request(
    project_id: int, req: CreatePRRequest, db: AsyncSession = Depends(get_db)
):
    result = await db.execute(select(Project).where(Project.id == project_id))
    project = result.scalar_one_or_none()
    if not project:
...
async def search_project(project_id: int, req: SearchRequest):
    try:
        ...
        return {
            "project_id": project_id, "query": req.query,
            "results": [], "error": f"Search unavailable: {str(e)}",
        }